...
- [ ] Frontend pages beyond the dashboard shells — campaign wizard, wallet/withdrawals UI, Instagram connect UI, admin queues, etc. This is the largest remaining gap; see the frontend build notes below.
- [ ] Database migrations — `schema.prisma` still has never had `prisma migrate dev` run against it (no Node.js on this machine), so `packages/db/prisma/migrations/` doesn't exist yet. The CI workflow above assumes it will once a real migration history exists.
- [ ] End-to-end verification — none of this has been run. It's correct-by-construction, not test-verified beyond the one unit suite above. Run `pnpm install && pnpm db:migrate` first — see `docs/development/ENVIRONMENT_SETUP.md`.
