...
- [x] Reel submission + async verification (`apps/api/src/modules/reels`) — a reel now sits at `PENDING_VERIFICATION` and a dedicated Reel Verification Worker resolves it via a real Graph API ownership check (matches the submitted URL against the clipper's own connected-account media list, the same check the Reel Detection Worker already trusted for auto-detected reels) plus a best-effort perceptual content-hash comparison (`MediaFingerprintService`, needs `ffmpeg` on PATH) against the brand's uploaded creative. Anything the worker can't resolve — API error, no fingerprint match, required mentions/hashtags — routes to `MANUAL_REVIEW`, never a silent pass.
...
- [x] Payments module (`apps/api/src/modules/payments`) — real, complete webhook signature verification (HMAC-SHA256 over the raw body) + normalized event dispatch to the wallet; provider is Razorpay, wired for real (`createDepositIntent`/`createPayout` call the actual Razorpay/RazorpayX SDK) — only throws `503 PAYMENTS_NOT_CONFIGURED` if `PAYMENT_PROVIDER_KEY`/`PAYMENT_PROVIDER_SECRET` aren't set in the environment
...
- [x] Background workers — 7 of 9 documented workers are real BullMQ workers sharing the API's service layer via a standalone Nest application context (`apps/api/src/worker.ts`): Instagram Sync, Reel Detection, Reel Verification, Metrics Sync, Earnings, Notification, Referral Reward expiry. Reel Verification moved off the inline/synchronous path once it started doing real external work (a Graph API call plus a content-hash download) — see the reel-submission entry above. Campaign Progress is deliberately live-computed by `AnalyticsService` rather than cached (see `docs/architecture/SCALABILITY_ARCHITECTURE.md` — caching it is "the first thing to revisit" once this stops being fast enough, not before).
- [x] Earnings are posted exactly once per reel — `PerformanceService.calculateForReel` used to re-fire on every hourly metrics-sync snapshot with no idempotency guard, so a single verified reel could be paid repeatedly until it drained its campaign's entire budget. Now gated on whether an earlier `PerformanceCalculation` row already exists for that reel, and the amount is the actual per-account rate the brand was quoted at funding time (`creatorBudget / maxParticipants`) rather than an arbitrary score-weighted fraction of the whole budget.
- [x] Eligible-clipper visibility (`GET /v1/campaigns/eligible-count`) — the create-campaign wizard now shows how many clippers currently match a campaign's category targeting, live, next to the accounts-wanted input — previously a brand could size a campaign against inventory that didn't exist with zero warning.
- [x] Earnings settlement window corrected to the spec'd 15 days (`earnings_settlement_window_days`, was seeded at 7).
