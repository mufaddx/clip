...
export default function CategoriesPage() {
  return (
    <main className="mx-auto max-w-3xl px-6 py-24">
      ...
      <p className="mt-4 text-slate-600">Campaigns and creators are organized by content category to keep matching relevant.</p>
      <div className="mt-8 flex flex-wrap gap-2">
        {CATEGORIES.map((c) => (
          <span key={c} className="rounded-full bg-slate-100 px-4 py-1.5 text-sm text-slate-700">{c}</span>
        ))}
      </div>
    </main>
  );
